...
export function ChapterIndicator() {
  ...
  return (
    <div className="fixed left-12 top-1/2 -translate-y-1/2 z-[100] hidden 2xl:flex flex-col gap-8 mix-blend-difference pointer-events-none">
      {CHAPTERS.map((chapter) => (
        <div 
          key={chapter.id}
          className={`flex items-center gap-6 transition-all duration-700 ease-in-out ${active === chapter.id ? "opacity-100 translate-x-0" : "opacity-30 -translate-x-2"}`}
        >
          <span className="text-[#F5F0E6] font-serif text-[14px] tracking-[0.2em]">{chapter.num}</span>
